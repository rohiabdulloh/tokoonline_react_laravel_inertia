...
const Kategori = (props) => {
  ...
  function loadMore(){
    axios.get(route('home.load', [page.jmlItem, props.nama_kategori, 'none'])).then((res) =>{
  ...
  return (
    <Layout>
      <Helmet>
        <title>{props.nama_kategori}</title>
      </Helmet>
      
        {/*tampilkan pesan jika kategori tidak ada*/}
        {props.kategori == null && ( 
            <Typography variant="h4">
                Tidak ditemukan kategori {props.nama_kategori}
            </Typography>
        )}
         {/*tampilkan nama kategori yang sedang ditampilkan*/}
         {props.kategori != null && (
            <Typography variant="h4">
                Kategori: {props.nama_kategori}
            </Typography>
        )}
